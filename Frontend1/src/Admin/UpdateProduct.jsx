...
const UpdateProduct = () => {
    ...
    useEffect(() => {
        if (product) {
            ...
            // Normalize category into an array for checkbox UI
            let normalizedCategory = [];
            const prodCat = product.category;
            if (Array.isArray(prodCat)) {
                normalizedCategory = prodCat;
            } else if (typeof prodCat === "string") {
                const trimmed = prodCat.trim();
                if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
                    try {
                        const parsed = JSON.parse(trimmed);
                        if (Array.isArray(parsed)) {
                            normalizedCategory = parsed;
                        } else if (typeof parsed === "string") {
                            normalizedCategory = parsed.split(",").map((c) => c.trim()).filter(Boolean);
                        }
                    } catch (e) {
